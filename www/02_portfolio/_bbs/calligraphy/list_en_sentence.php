<?php
	...
	$query = "SELECT * FROM callimania WHERE language = 'English' AND category = 'Sentence' ORDER BY id DESC LIMIT $no, $page_size";
	$result = mysql_query($query, $conn);

	// 총 게시물 수 를 구한다.
	$result_count=mysql_query("SELECT count(*) FROM callimania WHERE language = 'English' AND category = 'Sentence'", $conn);
...
<div id="contents">
	<p class="tab" align="center" style="width:900px; margin:0 auto 20px auto;">
		...
		<label><a href="./list_en_word.php"><input type="radio" name="genType" disabled /> English / Word</a></label>
		<label><a href="./list_en_sentence.php"><input type="radio" name="genType" checked /> English / Sentence</a></label>
	</p> <!--// tab -->
	<br />
